...
def add_front_obstacles(ego_pose, dist, obstacle):
    ego_yaw = ego_pose[-1]
    angle_bias = random.uniform(-np.pi/3, np.pi/3)
    target_pitch = random.uniform(-np.pi, np.pi)
    target_roll = random.uniform(-np.pi, np.pi)
    target_yaw = ego_yaw+angle_bias
    target_x = ego_pose[0]+dist*math.cos(target_yaw)
    target_y = ego_pose[1]+dist*math.sin(target_yaw)
    target_name = add_object_from_file(obstacle, (target_pitch, target_roll, target_yaw), (target_x, target_y, 0), 'obstacle')
...
def main(args):
    ...
    # add_puddle = random.choice([True, False])
    # if add_puddle:
    #     set_puddle('Next_Asphalt_Realistic', 0.5)
    set_background(args.hdri_path)
    bev_img = cv2.imread('/home/sczone/disk1/share/3d/blender_slots/lane_marker/untitled1.png')
    # bpy.ops.wm.save_mainfile(filepath='/home/sczone/disk1/share/3d/blender_slots/lane_marker/test2.blend')
    if not os.path.exists(args.ego_pose_path):
        set_ego_poses(bev_img, args.ego_pose_path, 250)
    render_img_mask(os.path.join(args.out_path, 'images'), args.ego_pose_path)
    bpy.ops.wm.open_mainfile(filepath='/home/sczone/disk1/share/3d/blender_slots/lane_marker/test.blend')
    render_img_mask(os.path.join(args.out_path, 'masks'), args.ego_pose_path)
